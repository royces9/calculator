...
double sya(char inp[]){
  ...
  int ii = 0, jj = 0, error = 0, cLEP = 0, cREP = 0;
  ...
  for(ii = 0; inp[ii]; ++ii){
    ...
    case ')':
      cREP++;
      while((oper.stk[oper.top] != LEP || oper.stk[oper.top]) && oper.occ != 0){
	exec(&out, popch(&oper));
      }
      popch(&oper);
      break;
...
int main(){
  char input[1024], *inputhistory[256]; //input string
  ...
  while(input != "q"){
    printw(">>"); // separator to know when to put input
    refresh();
    scanw("%s", &input);
