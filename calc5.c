...
double sya(char inp[]){
  ...
  for(ii = 0; inp[ii]; ++ii){
    ...
    case '^':
      if(oper.stk[oper.top] == '~' || oper.stk[oper.top] == '!' || oper.stk[oper.top] == '@' || oper.stk[oper.top] == '#' || oper.stk[oper.top] == '$'){
	exec(&out, popch(&oper));
	pushch(ch, &oper);
      }
    case '(':
    ...
    case 'h': break;
    case 'i':
      if((ach == A || ach == S || ach == M || ach == D || ach == EXP || ach == LEP || ach == REP || ach  == '\0') && bch == p){
	pushn(PI, &out);
      }	
      break;
    case 'j': break;
